...
async def register_couple(couple: CoupleRegistration):
    ...
    existing = await db.couples.find_one({"$or": [{"person1.email": couple.person1.email}, {"person2.email": couple.person2.email}], "status": "active"})
    if existing:
        raise HTTPException(status_code=400, detail="Email already registered")
    couple_id = str(uuid.uuid4())
    doc = {"_id": couple_id, "person1": couple.person1.dict(), "person2": couple.person2.dict(), "photos": couple.photos, "relationship_start_date": couple.relationship_start_date, "status": "active", "created_at": datetime.utcnow().isoformat()}
...
async def search(name: Optional[str] = None, state: Optional[str] = None, age: Optional[int] = None):
    ...
    if state:
        query["$or"] = [{"person1.state": {"$regex": state, "$options": "i"}}, {"person2.state": {"$regex": state, "$options": "i"}}]
    results = []
    async for doc in db.couples.find(query).limit(20):
        results.append({"id": doc["_id"], "person1": {"name": doc["person1"]["birth_name"], "state": doc["person1"]["state"], "age": doc["person1"]["age"]}, "person2": {"name": doc["person2"]["birth_name"], "state": doc["person2"]["state"], "age": doc["person2"]["age"]}, "relationship_start_date": doc.get("relationship_start_date"), "has_photos": len(doc.get("photos", [])) > 0})
...
async def request_verification(data: dict):
    ...
    await db.verifications.update_one({"email": email}, {"$set": {"code": code, "expires_at": (datetime.utcnow() + timedelta(minutes=10)).isoformat()}}, upsert=True)
    if SENDGRID_API_KEY:
        requests.post("https://api.sendgrid.com/v3/mail/send", headers={"Authorization": f"Bearer {SENDGRID_API_KEY}", "Content-Type": "application/json"}, json={"personalizations": [{"to": [{"email": email}]}], "from": {"email": SENDER_EMAIL}, "subject": "STATUS Verification Code", "content": [{"type": "text/plain", "value": f"Your code: {code}"}]})
